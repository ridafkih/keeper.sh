...
const PAGE_DESCRIPTION =
  "Free ICS file viewer. Open a .ics or iCalendar file and read its events as plain dates, times, and details. It runs in your browser, so the file never leaves your machine.";

const FAQ_ITEMS = [
  {
    question: "What is inside an ICS file?",
    answer: "Plain text in the iCalendar format from RFC 5545: a VCALENDAR block holding VEVENT entries, each with properties such as UID, DTSTART, DTEND, SUMMARY, LOCATION, and DESCRIPTION. This viewer reads those properties and lays them out in a readable form.",
  },
  {
...
function EventCard({ event }: { event: ParsedIcsEvent }) {
  return (
    <article className="flex flex-col gap-3 rounded-xl border border-interactive-border bg-background p-4">
      ...
      {event.end && <EventDetail label="Ends">{formatIcsDateValue(event.end)}</EventDetail>}
