...
Keeper.sh is in the same territory on Pro. Google and Microsoft both notify us the moment an event changes, and the copy is written as that notification arrives, so a change reaches your other calendar within seconds rather than on the next check.

On Free the timer still applies: your calendars are read every minute and the copies are written every 30, so a change lands in about fifteen minutes, thirty-one at the outside.
...
On Pro, Google and Outlook changes are written out within seconds of the provider telling us about them. On Free everything is read every minute and written every 30. Comparisons that call Keeper.sh a 30-minute product are describing the free plan's writes.
