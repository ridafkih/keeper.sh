...
On Pro, Google and Microsoft notify Keeper.sh the moment an event changes, so the copy is written within seconds. iCloud, Fastmail and other CalDAV calendars are read every minute instead, because Apple publishes no way for a calendar tool to be told an event changed. On Pro a new meeting lands on your other calendar as it is booked. On the free plan it is usually about fifteen minutes, and thirty-one at the very most.
