...
On Pro, Google and Microsoft notify Keeper.sh the moment an event changes, so the copy is written within seconds. iCloud, Fastmail and other CalDAV calendars are read every minute instead, because Apple publishes no way for a calendar tool to be told an event changed. On free everything is read every minute and copied every 30, so a change lands in about fifteen minutes, thirty-one at the outside.
