...
export const POST = withWideEvent(
  withV1Auth(async ({ params, userId }) => {
    ...
    const result = await keeperApi.rsvpEvent(userId, eventId, "tentative");

    if (!result.success) {
      return ErrorResponse.badRequest(result.error ?? "Failed to respond to event.").toResponse();
    }

    return Response.json({ status: "tentative" });
  }),
);
