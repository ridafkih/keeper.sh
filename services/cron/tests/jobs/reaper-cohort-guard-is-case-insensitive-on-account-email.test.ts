...
describe("counting co-holders of a google account behind oauth grant residue", () => {
  ...
  it("defers instead of revoking when only a mismatched email separates the surviving row", async () => {
    await insertSurvivor("someone-else@gmail.com");

    const census = await countSurvivingAccountLinks(database, residueFor("bob@gmail.com"));

    expect({
      blocked: census.blockingCredentialIds.length,
      coHolders: census.coHolders,
