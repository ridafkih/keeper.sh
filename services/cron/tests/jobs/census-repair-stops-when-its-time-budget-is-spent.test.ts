...
const LIVE_CUSTOMER_COUNT = 5;
const REPAIR_BUDGET_MS = 30_000;
const SECONDS_SPENT_PER_ROW_MS = 20_000;
const ROWS_THE_BUDGET_AFFORDS = 2;
const ROWS_LEFT_UNATTEMPTED = LIVE_CUSTOMER_COUNT - ROWS_THE_BUDGET_AFFORDS;
const CLOCK_ORIGIN_MS = Date.UTC(2026, 7, 25, 6, 15, 33, 956);
...
const createBrownedOutProviderClock = () => {
  const spent = { elapsedMs: 0 };

  return {
    now: () => new Date(CLOCK_ORIGIN_MS + spent.elapsedMs),
    spendOneSlowRoundTrip: () => {
      spent.elapsedMs += SECONDS_SPENT_PER_ROW_MS;
    },
  };
};

const stubUserInfoThatCostsWallTime = (clock: {
  spendOneSlowRoundTrip: () => void;
}): void => {
...
describe("census repair stops when its time budget is spent", () => {
  beforeEach(async () => {
  ...
  it("attempts only the rows the budget affords and reports the rest as unattempted", async () => {
    const clock = createBrownedOutProviderClock();
    stubUserInfoThatCostsWallTime(clock);
    ...
    const tally = await repairCensusBlockingCredentials(
      ...
      { budgetMs: REPAIR_BUDGET_MS, now: clock.now },
    );

    expect(recorded.userInfoTokens).toHaveLength(ROWS_THE_BUDGET_AFFORDS);
    expect(tally.attempted).toBe(ROWS_THE_BUDGET_AFFORDS);
    expect(tally.repaired).toBe(ROWS_THE_BUDGET_AFFORDS);
    expect(tally.remaining).toBe(ROWS_LEFT_UNATTEMPTED);
    expect(await countRepairedCalendarRows()).toBe(ROWS_THE_BUDGET_AFFORDS);
