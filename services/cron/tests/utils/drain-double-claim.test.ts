...
const fakeRedis = {
  ...
  get: (key: string): Promise<string | null> => Promise.resolve(strings.get(key) ?? null),
  ...
  set: (key: string, value: string, ...options: (string | number)[]): Promise<string | null> => {
...
describe("a second drain skips a calendar already being drained", () => {
  it("leaves the in-flight calendar to its holder and drains the rest of the batch", async () => {
