...
let scenario: "revoked-grant" | "userinfo-brownout" = "revoked-grant";
...
vi.mock("../../src/context", () => ({
  ...
  oauthProviders: {
    getProvider: () => ({
      ...
      fetchUserInfo: () => Promise.reject(new Error("userinfo 503: backend error")),
...
const credentialRowFor = (scenarioName: typeof scenario) => ({
  accessToken: scenarioName === "revoked-grant" ? "stale-access-token" : "access-token-1",
  email: "person@example.com",
  expiresAt: new Date(Date.now() + (scenarioName === "revoked-grant" ? 30_000 : 3_600_000)),
  needsReauthentication: false,
  provider: "google",
  refreshToken: scenarioName === "revoked-grant" ? "revoked-refresh-token" : "refresh-token-1",
});
...
const connect = () => {
  selectResults = [[credentialRowFor(scenario)], [], [], []];

  return createOAuthSource({
...
describe("Connect result reports that the new account needs reauthentication", () => {
  it("reports needsReauthentication on a terminal invalid_grant", async () => {
  ...
  it("does not report needsReauthentication for a transient userinfo brownout", async () => {
    scenario = "userinfo-brownout";

    const source = await connect();

    expect(refreshRejections).toHaveLength(0);
    expect(telemetryFields).toContainEqual([
      "oauth_source.provider_account_id_resolution",
      "provider_failure",
    ]);
    expect(source.id).toBe("source-1");
