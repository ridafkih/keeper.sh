...
const LIVE_PROVIDER_CHANNEL_ID = "google-stopped-1";
...
const seedChannels = (): StoredPushChannel[] => [
  {
    ...
    nextAttemptAt: null,
    provider: "google",
    providerChannelId: LIVE_PROVIDER_CHANNEL_ID,
...
describe("push channel residue on a replica with no webhook url configured", () => {
  it("keeps the residue of every channel the delete never dialed", async () => {
    ...
    expect(survivingChannelIds).toEqual([LIVE_PROVIDER_CHANNEL_ID]);
  });
});
