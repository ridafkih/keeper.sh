...
const PROVIDER_OUTAGE = "google returned 500 for channels.stop";
const REDIS_BLIP = "redis blip: CONNECTION_BROKEN";
...
const createRedisHarness = () => {
  ...
  return {
    ...
    redis: {
      del: () => Promise.reject(new Error(REDIS_BLIP)),
...
describe("a rollback whose tombstone erase fails still discards the residue it recorded", () => {
  it("leaves no residue behind for a surviving user even when redis rejects the del", async () => {
    ...
    const dependencies = {
      ...
      listOAuthCredentials: () =>
        Promise.resolve([
          {
            accessToken: liveCredential.accessToken,
            accountId: "account-1",
            email: "owner@example.com",
            provider: "google",
            refreshToken: liveCredential.refreshToken,
    ...
    expect(residue.kindsFor(ROLLED_BACK_USER)).toEqual([OAUTH_GRANT_KIND, PUSH_CHANNEL_KIND]);
    expect(
      residue
        .rowsFor(ROLLED_BACK_USER)
        .find((row) => row.kind === PUSH_CHANNEL_KIND)?.providerChannelId,
    ).toBe(ABANDONED_CHANNEL_ID);

    const rollbackFailure = await createDeleteUserSyncTeardownRollback(dependencies as never)(
      ROLLED_BACK_USER,
    ).then(
      () => undefined,
      (error: unknown) => error,
    ...
    expect(rollbackFailure).toBeInstanceOf(Error);
    expect((rollbackFailure as Error).message).toContain(REDIS_BLIP);

    expect(residue.rowsFor(ROLLED_BACK_USER)).toEqual([]);

    expect(
      capture.loggedFields.some((fields) => "delete_user.residue_discarded" in fields),
    ).toBe(true);
  });
});
