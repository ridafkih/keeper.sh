...
describe("Connect inserts the account already flagged for reauthentication", () => {
  it("writes the reauthentication flag on the calendar account row it creates", async () => {
    await connect();
    ...
    expect(accountInserts[0]?.values.needsReauthentication).toBe(true);
    expect(accountInserts[0]?.values.reauthenticationSource).toBe("token-refresh");
  });
});
