...
| PRIVATE_RESOLUTION_WHITELIST          | `api`, `cron` | Optional. When `BLOCK_PRIVATE_RESOLUTION` is `true`, this comma-separated list of hosts is exempt from the restriction. Each entry is matched against the URL host exactly, so include the port whenever the calendar is served on a non-default one.<br><br>e.g. `192.168.1.50,radicale.local:5232,10.0.2.12:8080` |
