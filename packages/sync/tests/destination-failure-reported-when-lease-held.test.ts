...
describe("an owned run still reports exactly as it does today", () => {
  it("returns one error and fires the callback once when the lease is still held", async () => {
    ...
    expect(result.errors).toEqual(["read ECONNRESET"]);
