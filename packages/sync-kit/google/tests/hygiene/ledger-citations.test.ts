...
const googleSectionOf = (ledger: string): string =>
  ledger.slice(ledger.indexOf("### GOOG-I1."));

const citationsIn = (section: string): readonly Citation[] =>
  [...section.matchAll(/`google\/(tests\/[^`\s]+\.test(?:-d)?\.ts)\s*::\s*([^`]+)`/g)].flatMap(
    (match) => {
...
const citations = citationsIn(googleSectionOf(await ledgerText()));

describe("the ledger can be walked against the suite", () => {
  test("GOOG-H2: every cited test file exists", async () => {
  ...
  test("GOOG-H2: every cited test name exists verbatim in the file that is cited", async () => {
    const broken: string[] = [];
  ...
  test("GOOG-H2: the walk covers the whole ledger, not a handful of lines", () => {
