...
const caldavSectionOf = (ledger: string): string => ledger.slice(ledger.indexOf("### DAV-I1."));

const citationsIn = (section: string): readonly Citation[] =>
  [...section.matchAll(/`caldav\/(tests\/[^`\s]+\.test(?:-d)?\.ts)\s*::\s*([^`]+)`/g)].flatMap(
    (match) => {
...
const citations = citationsIn(caldavSectionOf(await ledgerText()));

describe("the ledger can be walked against the suite", () => {
  test("DAV-H23: every cited test file exists", async () => {
  ...
  test("DAV-H23: every cited test name exists verbatim in the file that is cited", async () => {
    const broken: string[] = [];
  ...
  test("DAV-H23: the walk covers the whole ledger, not a handful of lines", () => {
    expect(citations.length).toBeGreaterThan(80);
  });
});
