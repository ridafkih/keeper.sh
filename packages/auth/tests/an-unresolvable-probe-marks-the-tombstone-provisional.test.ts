...
type ProbeOutcome = "row_gone" | "unresolvable";
...
const buildHarness = (probeOutcome: ProbeOutcome): Promise<DeletionHarness> => {
  ...
  return auth.$context.then((context) => {
    ...
    internalAdapter.findUserById = () => {
      if (probeOutcome === "unresolvable") {
        return Promise.reject(new Error(PROBE_FAILURE));
      }
...
describe("an unresolvable probe marks the tombstone provisional", () => {
  it("writes an unconfirmed companion key beside the surviving tombstone", async () => {
    const harness = await buildHarness("unresolvable");

    await requestAccountDeletion(harness.auth);

    expect(harness.tombstones.keys().toSorted()).toEqual(
      [PROVISIONAL_KEY, TOMBSTONE_KEY].toSorted(),
    );
  ...
  it("leaves no companion key when the probe resolves the row as gone", async () => {
    const harness = await buildHarness("row_gone");
