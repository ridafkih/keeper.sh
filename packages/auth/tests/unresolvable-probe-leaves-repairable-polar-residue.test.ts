...
describe("an unresolvable probe leaves repairable polar residue", () => {
  it("records a polar_customer residue row the reaper can repair", async () => {
    ...
    expect(harness.residue).toContainEqual(
      expect.objectContaining({ externalId: USER_ID, kind: "polar_customer" }),
    );
  });
});
