...
class TruncatingDestinationWithBlindLookup {
  ...
  public readonly lookedUpIds: string[] = [];
  ...
  public getRemoteEventsByIds = (eventIds: string[]): Promise<RemoteEvent[]> => {
    this.lookedUpIds.push(...eventIds);
...
const reconcile = (
  destination: TruncatingDestinationWithBlindLookup,
...
describe("a normalising provider settles even when the lookup returns no row", () => {
  it("records what it observes and emits nothing thereafter", async () => {
    const destination = new TruncatingDestinationWithBlindLookup();
