...
class NormalisingDestination {
  ...
  public pushEvents = (events: MaterializedSyncableEvent[]): Promise<PushResult[]> =>
    Promise.resolve(events.map((event): PushResult => {
      ...
      const uid = `remote-uid-${this.stored.size + 1}`;
      this.stored.set(uid, {
...
const buildLegacyState = (): { destination: NormalisingDestination; mappingStore: MappingStore } => {
  const destination = new NormalisingDestination();
  destination.seed({
    description: OVERLONG_DESCRIPTION.slice(0, DESTINATION_DESCRIPTION_LIMIT),
    endTime: LOCAL_EVENT.endTime,
    location: LOCAL_EVENT.location ?? "",
    startTime: LOCAL_EVENT.startTime,
    summary: LOCAL_EVENT.summary,
    uid: LEGACY_REMOTE_UID,
  });
...
const reconcile = (destination: NormalisingDestination, mappingStore: MappingStore) =>
  syncCalendar({
    calendarId: DESTINATION_CALENDAR_ID,
...
describe("a mapping written before the provider form was recorded", () => {
  it("records the provider form on first sight without treating it as a change", async () => {
    const { destination, mappingStore } = buildLegacyState();
  ...
  it("still detects a genuine third-party edit once the provider form is recorded", async () => {
    const { destination, mappingStore } = buildLegacyState();

    await reconcile(destination, mappingStore);

    destination.editStoredSummary(OWNER_EDITED_SUMMARY);
    destination.resetCalls();
    ...
    expect(restored?.editableContent?.summary).toBe(LOCAL_EVENT.summary);
  });
});
