...
class NormalisingDestination {
  ...
  public readonly multigetBatches: string[][] = [];
  ...
  public getRemoteEventsByIds = (eventIds: string[]): Promise<RemoteEvent[]> => {
    this.multigetBatches.push([...eventIds]);
...
describe("a person editing our mirrored event is still detected", () => {
  it("detects and repairs an owner edit made before any later reconcile recorded the remote form", async () => {
    ...
    expect(written?.editableContent?.summary).toBe(LOCAL_EVENT.summary);
  ...
  it("goes quiet again after the repair instead of churning", async () => {
    const destination = new NormalisingDestination();
    const mappingStore = new MappingStore();
