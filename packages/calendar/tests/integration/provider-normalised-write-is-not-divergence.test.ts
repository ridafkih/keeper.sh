...
/*
 * Stands in for a destination that stores a normalised version of what it was
 * sent, the way Google clips a description at its 8 KB limit.
 */
...
const reconcile = (destination: TruncatingDestination, mappingStore: MappingStore) =>
  syncCalendar({
...
describe("a provider that normalises what we wrote does not look changed", () => {
  it("emits no operations on the reconcile after the destination truncated our description", async () => {
    ...
    expect(remote?.editableContent?.description).toHaveLength(DESTINATION_DESCRIPTION_LIMIT);
  ...
  it("still detects a genuine third-party edit of the mirrored event", async () => {
    const destination = new TruncatingDestination();
    const mappingStore = new MappingStore();
    ...
    destination.editStoredSummary("Renamed by the calendar owner");
    destination.resetCalls();
    ...
    expect(destination.pushedEventIds).toEqual([LOCAL_EVENT.id]);
    expect(destination.deletedIds).toHaveLength(1);
