...
describe("an event the recipient deleted is restored", () => {
  it("recreates a mirror missing from the listing even when the write answers 2xx", async () => {
    ...
    expect(destination.snapshot().map((record) => record.summary)).toEqual(["Team lunch"]);
    ...
    const liveDeleteIds = destination.snapshot().map((record) => record.deleteId);

    expect(mappedDeleteIds).toHaveLength(1);
    for (const deleteId of mappedDeleteIds) {
      expect(liveDeleteIds).toContain(deleteId);
    }
  });
