...
const revocableGrantForGoneUserTwo = (): TeardownResidueRecord => ({
  accountEmail: "gone-two@example.com",
  attempts: 0,
  createdAt: RECORDED_AT,
  credential: {
...
const createHarness = () => {
  ...
  return { clearAttempts, deregistered, errors, observed, reap, revocations };
};
...
describe("a failing residue clear does not exile the rest of the batch", () => {
  it("reports the rejecting clear, keeps processing the batch, and still observes the pass", async () => {
    ...
    expect(harness.revocations).toEqual([
      { residueId: "residue-grant-user-2", token: "refresh-token-user-2" },
    ]);
    expect(outcome.clearedIds).toContain("residue-grant-user-2");
